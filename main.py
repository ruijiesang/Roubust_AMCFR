...
def main(cfgs):
    ...
    if cfgs.params["loss"]=="loss_FG_test2":
        criterion = init_loss_FG(cfgs.params["loss"],len(cfgs.mod_type),cfgs.params["loss_t"],cfgs.params["loss_alp"])
    else:
    ...
    if cfgs.train:
        ...
        trainer = Trainer(model=model, device=device, optimizer=optimizer, lr_decay=cfgs.params["lr_decay"], criterion=criterion,
                          save_path='./results_12.14/' + cfgs.method + '/' + cfgs.params["loss"] + '/' + cfgs.dataset + '/checkpoints',
        ...
        draw_train(train_loss, train_acc, valid_loss, valid_acc,
                   save_path='./results_12.14/' + cfgs.method + '/' + cfgs.params["loss"] + '/' + cfgs.dataset + '/draws')
    ...
    # 测试下总体混淆矩阵
    draw_conf(test_conf, save_path='./results_12.14/' + cfgs.method + '/' + cfgs.params["loss"] + '/' + cfgs.dataset + '/draws',
