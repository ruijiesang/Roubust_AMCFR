...
def draw_train(train_loss, train_acc, valid_loss, valid_acc, save_path):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(train_loss[5:], "r-", label="Train loss")
    plt.plot(valid_loss[5:], "b--", label="Val loss")
    plt.legend()
    plt.xlabel("epoch")
    plt.ylabel("loss")

    plt.subplot(1, 2, 2)
    plt.plot(train_acc[5:], "ro-", label="Train acc")
    plt.plot(valid_acc[5:], "bs-", label="Val acc")
    plt.xlabel("epoch")
    plt.ylabel("acc")
    plt.legend()
...
def draw_acc(snrs, test_acc_snr, save_path):
    ...
    plt.plot(snrs, test_acc_snr, "b-", label="Test acc")
    plt.xlabel("snr(dB)")
    plt.ylabel("acc")
    plt.legend()
    os.makedirs(save_path, exist_ok=True)
