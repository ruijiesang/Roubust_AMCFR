...
def main(cfgs,t):
    ...
    # 单个模型训练
    if cfgs.train:
        optimizer = torch.optim.AdamW(model.parameters(), lr=float(cfgs.params["lr"]),
                                      weight_decay=cfgs.params["weight_decay"])
        trainer = Trainer(model=model, device=device, optimizer=optimizer, lr_decay=cfgs.params["lr_decay"],
                          criterion=criterion,
                          save_path=path,
...
if __name__ == '__main__':
    ...
    best_acc=0
    for t1 in t:
        cfgs = get_cfgs_2016aCen_noisydatatest_2()
